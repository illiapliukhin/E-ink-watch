"""R_SCL -0.25 + TS spur del + PMIC_INT via to (109.5,107.05) clear of VBAT."""
import pcbnew
...
for t in tracks:
    ...
    if cls == "PCB_VIA" and net == "PMIC_INT":
        ...
        if abs(x - 109.500) < 0.05 and abs(y - 106.400) < 0.05:
            t.SetPosition(pcbnew.VECTOR2I(mm(NEW_VIA[0]), mm(NEW_VIA[1]))); n += 1
            print(f"PMIC_INT via -> {NEW_VIA}")
        continue
    if cls == "PCB_VIA":
        continue
    if net in ALLOW:
        ...
        if net == "3V3" and t.GetLayer() == pcbnew.F_Cu:
            sx = tomm(t.GetStart().x); ex = tomm(t.GetEnd().x)
            sy = tomm(t.GetStart().y); ey = tomm(t.GetEnd().y)
    ...
    if net == "TS" and t.GetLayer() == pcbnew.F_Cu:
        ...
        if ((abs(sx - 107.690) < 0.05 and abs(sy - 107.550) < 0.05 and
            ...
            to_remove.append(t)
    if net == "PMIC_INT" and t.GetLayer() == pcbnew.F_Cu:
        sx, sy = tomm(t.GetStart().x), tomm(t.GetStart().y)
        ex, ey = tomm(t.GetEnd().x), tomm(t.GetEnd().y)
...
for x1, y1, x2, y2, w in [
    (110.600, 106.400, 110.600, NEW_VIA[1], 0.15),
    (110.600, NEW_VIA[1], NEW_VIA[0], NEW_VIA[1], 0.15),
]:
