#!/usr/bin/env python3
"""Pass-X25: CD escape west then B — clear TS @x=111 and R_SDA."""
import sys, re
...
for a,b,c,d in [
    (110.60, 106.80, 110.60, 107.20),
    (110.60, 107.20, 110.60, 108.50),
    (110.60, 108.50, 113.30, 108.50),
    (113.30, 108.50, 113.30, 108.41),
]:
    t, k = del_segments_at(t, a, b, c, d)
    if k: print(f"del {k}")
t, n = del_via_xy(t, 110.60, 107.20)
print(f"del via {n}")

cd = netnum(t, "CD")
# West along pad row carefully — E1 is no-net at 110.2; go south first to 107.0 then west
t = add_segment(t, 110.60, 106.80, 110.60, 107.00, 0.15, "F.Cu", cd)
t = add_segment(t, 110.60, 107.00, 109.70, 107.00, 0.15, "F.Cu", cd)
t = add_via(t, 109.70, 107.00, 0.40, 0.25, cd)
t = add_segment(t, 109.70, 107.00, 109.70, 108.50, 0.18, "B.Cu", cd)
