#!/usr/bin/env python3
"""DRC gate for Pass-AE. Usage: _pass_ae_gate.py <label>"""
import re, sys, shutil
from pathlib import Path
from collections import Counter

label = sys.argv[1]
text = Path(f"reports/_drc_passae_{label}.txt").read_text()
pairs = Counter()
...
if s or not pok or not pocket:
    shutil.copy("backups/_passae_last_ok.kicad_pcb", "e-ink-watch.kicad_pcb")
    print("REVERT")
    for b in re.split(r"\[shorting_items\]:", text)[1:][:5]:
        print(b.split("\n[")[0][:320])
    sys.exit(1)
shutil.copy("e-ink-watch.kicad_pcb", "backups/_passae_last_ok.kicad_pcb")
print("KEEP")
# stitch check
