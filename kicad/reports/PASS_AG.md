...
GND A5–D5, west 3V3, west 3V3_DISP, SDA to U1, SCL E5 vs `R_SCL` pad1, SCL to U1, PMIC_INT, TS C3 vs TS B via `(107.8, 105.65)`. Pocket CD (E2) and ILIM (C2) are closed.

## Next

Orthogonal F/B reconnect of TS C3 and SCL E5→`R_SCL` pad1. New CD B.Cu at `y=106.95` `x=110.60–112.65` — through vias there short CD. No In1/In2. No new overlap. East 0402 column stays (`R_CD`/`C_LDO` x=113.5, `R_LSCTRL` x=114.6, `R_ILIM` x=115.6).
